...
from wk2022.models import Bids, Player, League, GamePhase, Boekhouding, BoekhoudingLeague, Opstelling, OpstellingLog, \
...
def standenbeheer(request):
    ...
    return render(
        request,
        template_name="wk2022/standenbeheer.html",
...
def add_goal(request, pk: int):
    ...
    return render(
        request=request, template_name="wk2022/add_goal.html", context={"form": form}
    )
...
def changefirstname(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, "wk2022/changefirstname.html", {'form': ChangeFirstNameForm})
    #
    else:
        return render(request, template_name="wk2022/changefirstname.html")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                if teamtochange:
                    error = "teamexists"
                    return render(request, "wk2022/changeteamname.html", context={"error": error})
            except:
                if teamcurrentuser:
            ...
            return render(request, "wk2022/changeteamname.html", context={'form': ChangeTeamNameForm, "error": error})

    else:
        return render(request, template_name="wk2022/changeteamname.html")
...
def myteam(request):
    ...
    try:
        ...
        return render(request, template_name="wk2022/myteam.html",
...
def myleague(request):
    ...
    return render(request, template_name="wk2022/myleague.html",
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if leaguename == "Kies een league":
                ...
                return render(request, template_name="wk2022/pickleague.html",
...
def listallbids(request):
    ...
    try:
        ...
        return render(request, 'wk2022/listallbids.html',
...
def groepstand(request):
    ...
    return render(
        request=request,
        template_name="wk2022/groepstand.html",
...
def listauction(request):
    ...
    return render(request, 'wk2022/auctionoverview.html',
    ...
    if request.method == 'POST':
        if request.POST.get("listauction"):
            ...
            allbids = Bids.objects.select_related('player').filter(team__in=list(teams.values_list()), gamephase=auctionphase.pk).order_by(
                "-playerbid")
        return render(request, 'wk2022/listauction.html',
...
def bidmenu(request):
    ...
    return render(request, template_name="wk2022/bidmenu.html",
    ...
    if request.method == 'POST':
        if request.POST.get("bewaarveiling"):
            ...
            if team.betcoins < int(team.bidbudget):
                ...
                return render(request, "wk2022/bidoverview.html",
...
def bids(request, country_name):
    ...
    try:
        ...
        error = "Please create a team first. Go to the Change Team page located in My Account"
        return render(request, 'wk2022/bids.html',
...
def auction(request, league, gamephase):
    ...
    if not currentuser.is_superuser:
        ...
        return render(request, 'wk2022/assignedbidsperteam.html',
...
def rejectedbidsperteam(request, league):
    ...
    bids = Bids.objects.select_related('player').filter(team__in=list(teams.values_list()), assigned=False).order_by(
        '-gamephase', "-playerbid")
    return render(request, 'wk2022/rejectedbidsperteam.html',
...
def unassignedplayersperleague(request, league):
    ...
    #     if not any(bid['player_id'] == player.id for bid in assignedbids):
    #         unassignedplayers.append(player)
    return render(request, 'wk2022/unassignedplayersperleague.html',
...
def leagueoverview(request):
    ...
    return render(request, 'wk2022/leagueoverview.html',
                  context={'leagues': leagues, 'leaguedata': leaguedata})


def leaguemanager(request, league):
    currentleague = League.objects.get(pk=league)
    return render(request, 'wk2022/leaguemanager.html',
...
def teams(request, league):
    ...
    return render(request, 'wk2022/teams.html',
...
def changephase(request, league):
    ...
    return render(request, 'wk2022/changephase.html', context={'league': leaguename, 'currentphase': currentphase,
...
def createleague(request):
    ...
    return render(request, "wk2022/createleague.html", context={"form": CreateLeagueForm, "error": error})


def regels(request):
    return render(request, "wk2022/regels.html")


def programma(request):
    return render(request, "wk2022/programma.html")
...
def moneymanager(request):
    ...
    return render(request, "wk2022/moneymanager.html",
...
def tactiekopstelling(request):
    ...
    if len(currentlineup) != 11 and len(currentlineup) > 0:
        ...
        return render(request, "wk2022/tactiekopstelling.html",
...
def myledger(request):
    ...
    return render(request, "wk2022/myledger.html",
                  context={"jouwboekhouding": jouwboekhouding, "currentteam": currentteam})


def hulpbieden(request):
    return render(request, "wk2022/hulpbieden.html")
...
def minplayerpositions(request, league):
    ...
    return render(request, "wk2022/minplayerpositions.html", context={"error": error})
...
def fillteams(request, league):
    ...
    return render(request, "wk2022/fillteams.html", context={"error": error})
    ...
    for team in allteams:
        ...
        for attack in attacker:
            Opstelling.objects.create(team=team, opgesteldespeler=Player.objects.get(pk=attack.player_id),
                                      phase=leaguephase)
    return render(request, "wk2022/initiallineup.html", context={"error": error})
...
def livescoring(request):
    ...
    for virtualmatch in allewedstrijden:
        ...
        wedstrijdeninfo.append([virtualmatch, virtualmatch.decimalhomegoalscore + virtualmatch.decimalhomescore,
                                ...
                                awaytactiek])
    return render(request, "wk2022/livescoring.html",
...
def standvoorverlenging(request, league):
    error = ""
    opslaan = False
    scoring = False
    opstellingsinfo = []
    ...
    if "Opstelling" in currentleague.gamephase.gamephase or "Live" in currentleague.gamephase.gamephase:
        ...
        return render(request, "wk2022/standvoorverlenging.html",
                      context={"error": error, "groups": groups, "allewedstrijden": wedstrijdeninfo,
    ...
    for wedstrijd in allewedstrijden:
        wedstrijdinfo = resultaatperwedstrijd(wedstrijd, phasetext, currentstage, False, False, opslaan)
...
def spelersontslaan(request):
    ...
    if request.method == 'POST':
        if request.POST['selectontslaspeler'] == "nochoice":
            ...
            return render(request, "wk2022/spelersontslaan.html", context={"error": error, "jouwspelers": truebids})
...
def ontslaspelers(request, league, alleenlijst):
    ...
    if len(spelersteontslaan) == 0:
        ...
        return render(request, "wk2022/ontslaspelers.html",
...
def statusontslaan(request, league):
    ...
    if len(spelerstatusontslaan) == 0:
        ...
        return render(request, "wk2022/statusontslaan.html",
...
def keerpremieuit(request, league, alleenlijst):
    ...
    return render(request, "wk2022/keerpremieuit.html",
...
def rlkofase(request):
    ...
    groups = models.Country.Group.names
    return render(request, "wk2022/rlkofase.html",
...
def kofase(request):
    ...
    return render(request, "wk2022/kofase.html",
...
def listleaguebids(request, league):
    ...
    return render(request, "wk2022/listleaguebids.html",
...
def getlineups(request):
    error = ""
    lineups = readscreen()
    return render(request, "wk2022/getlineups.html",
...
def updatescores:
    ...
    for event in match_data['data']['events'][::-1]:
        if 'GOAL' in event['type']:
            print('{} minute: {} {}: {}-{} ({} points)'.format(
                event['time'], event['player']['firstName'], event['player']['lastName'], event['homeScore'],
                event['awayScore'],
